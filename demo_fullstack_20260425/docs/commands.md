...
```bash
cd backend
uv run demo_fullstack_20260425 <group> <command> [options]
```

### Server Commands

```bash
uv run demo_fullstack_20260425 server run              # Start dev server
uv run demo_fullstack_20260425 server run --reload     # With hot reload
uv run demo_fullstack_20260425 server run --port 9000  # Custom port
uv run demo_fullstack_20260425 server routes           # Show all registered routes
```

### Database Commands

```bash
uv run demo_fullstack_20260425 db init                  # Run all migrations
uv run demo_fullstack_20260425 db migrate -m "message"  # Create new migration
uv run demo_fullstack_20260425 db upgrade               # Apply pending migrations
uv run demo_fullstack_20260425 db upgrade --revision e3f  # Upgrade to specific revision
uv run demo_fullstack_20260425 db downgrade             # Rollback last migration
uv run demo_fullstack_20260425 db downgrade --revision base  # Rollback to start
uv run demo_fullstack_20260425 db current               # Show current revision
uv run demo_fullstack_20260425 db history               # Show migration history
```
...
uv run demo_fullstack_20260425 user create

# Create user non-interactively
uv run demo_fullstack_20260425 user create --email user@example.com --password secret

# Create user with specific role
uv run demo_fullstack_20260425 user create --email admin@example.com --password secret --role admin

# Create user with superuser flag
uv run demo_fullstack_20260425 user create --email admin@example.com --password secret --superuser
...
# List all users
uv run demo_fullstack_20260425 user list
```
...
```bash
uv run demo_fullstack_20260425 cmd <command-name> [options]
```
...
```bash
uv run demo_fullstack_20260425 cmd my-command --name test
```
